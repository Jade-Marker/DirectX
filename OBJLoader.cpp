...
bool OBJLoader::FindSimilarVertex(const LightVertex& vertex, std::map<LightVertex, unsigned short>& vertToIndexMap, unsigned short& index)
{
							  ...
							  std::vector<XMFLOAT3>& outNormals)
{
	...
	std::pair<LightVertex, unsigned short> pair;

	int numVertices = inVertices.size();
	
	for(int i = 0; i < numVertices; ++i) //For each vertex
	{
		LightVertex vertex = {inVertices[i], inNormals[i],  inTexCoords[i]}; 
...
Mesh* OBJLoader::Load(char* filename, ID3D11Device* _pd3dDevice, bool invertTexCoords)
{
	...
	if(!binaryInFile.good())
	{
		...
		else
		{
			...
			LightVertex* finalVerts = new LightVertex[meshVertices.size()];
			...
			outbin.write((char*)finalVerts, sizeof(LightVertex) * numMeshVertices);
	...
	else
	{
		...
		LightVertex* finalVerts = new LightVertex[numVertices];
