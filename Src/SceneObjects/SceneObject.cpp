...
SceneObject::SceneObject(const Transform& transform, XMFLOAT3 tScale, SceneObject* parent, Mesh* mesh, bool startInWireFrame, Shader* shader,
    ConstantBuffer* pLocalConstantBuffer, std::vector<Texture*> textures) :
	_transform(transform), _tScale(tScale), _parent(parent), _mesh(mesh), _shader(shader),
...
{
    if (_xDirState)
        _transform.Rotation.x += _tScale.x * deltaTime;
    else
        _transform.Rotation.x -= _tScale.x * deltaTime;

    if (_yDirState)
        _transform.Rotation.y += _tScale.y * deltaTime;
    else
        _transform.Rotation.y -= _tScale.y * deltaTime;

    _transform.Rotation.z += _tScale.z * deltaTime;
